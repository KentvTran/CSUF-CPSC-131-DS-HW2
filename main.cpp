...
int main()
{
  try
  {
    ...
    GroceryList dateNight = { { "pasta" },
                              { "butter" },
                              { "pecorino" } };

    std::cout << "Here's the ingredients I have so far for the Cacio e Pepe: " << dateNight << "\n\n";

    dateNight.moveToTop( { "steak" } );

    GroceryList vegetables = { { "asparagus" },
                               { "onion" }, };

    dateNight += { { "ramune" },
                   { "yakult" } };

    if( dateNight != vegetables ) dateNight += vegetables;

    dateNight.remove( { "ramune" } );

    GroceryList utensils = {
        { "plate" },
        { "knife" },
    };
    dateNight += utensils;

    std::cout << "Let's see what's in our list now: " << dateNight << "\n\n";
